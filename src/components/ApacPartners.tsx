...
// 75 partner logos exported from the design source, dealt into five rows.
// Rows share one direction — alternating them was tried and rejected (27865d17);
// the staggered durations are what keep the rows out of lockstep.
...
export function PartnerLogos() {
  return (
    <section className="bg-[#FCFCFC] border-b border-frame overflow-hidden">
      <div className="mt-8 frame:mt-0 frame:pt-[54px] flex flex-col gap-2 frame:gap-[9px]">
        {ROWS.map((row, r) => (
          <div key={r} className="overflow-hidden">
            <div className="cb-marquee flex items-center w-max" style={{ animationDuration: DURATIONS[r] }}>
              {[0, 1].map((copy) => (
  ...
  return (
    <section className="relative bg-[#FCFCFC] overflow-hidden">
      ...
      <div className="relative border-b border-frame pb-3.5 frame:pb-0">
        <span
          className="inline-flex items-center bg-violet-brand text-white text-lg frame:text-[19px] font-semibold pl-7 frame:pl-[60px] pr-12 frame:pr-[80px] py-5 frame:py-0 frame:min-h-[92px] frame:min-w-[319px]"
      ...
      <div className="relative pl-6 frame:pl-[60px] frame:pr-[20px] grid frame:grid-cols-[1fr_804px] gap-6 frame:gap-0 items-start">
        ...
        <div className="relative grid grid-cols-2 gap-3 frame:gap-x-[12px] frame:gap-y-[14px] frame:pt-[42px] pb-6 frame:pb-[36px]">
          {COUNTRIES.map((c) => (
            <div
              key={c.flag}
              className="flex items-center gap-4 frame:gap-[22px] rounded-[10px] bg-white/95 ring-1 ring-black/[0.06] shadow-[0_2px_10px_-4px_rgba(30,20,60,0.12)] px-4 frame:px-[26px] py-3 frame:py-[26px] frame:min-h-[107px]"
            >
